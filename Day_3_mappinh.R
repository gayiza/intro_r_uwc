...
#landmask
ggplot(data = south_africa_coast, aes(x = lon, y = lat)) +
  geom_polygon(colour = "black", fill = "grey60", aes(group = group)) # The land mask
...
ggplot(data = south_africa_coast, aes(x = lon, y = lat)) +
  ...
  coord_equal(xlim = c(15, 34), ylim = c(-36, -26), expand = 0) # Force lon/lat extend
...
ggplot(data = south_africa_coast, aes(x = lon, y = lat)) +
  ...
  coord_equal(xlim = c(15, 34), ylim = c(-36, -26), expand = 0)  #set limits
...
# [A.A]
# Script runs fully
# Add self made comments
# Neat script
# most of these coments and code seems copied from the document....
# PLay and explore the different functons
